...
export function setButtonLoading(button, loadingText = 'Loading...') {
  ...
  if (buttonId === 'refresh-rooms-btn') {
    // For refresh button, just add the rotating class to the existing SVG
...
export function restoreButton(button, originalContent, delay = 0) {
  setTimeout(() => {
    // Special handling for refresh button
    if (button.id === 'refresh-rooms-btn') {
      // For refresh button, just remove the rotating class
      const svg = button.querySelector('svg');
      if (svg) {
